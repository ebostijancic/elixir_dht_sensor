...
static ERL_NIF_TERM setup(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[])
{
    ...
    if(setuid(getuid()) < 0) exit(EXIT_FAILURE);

    return enif_make_atom(env, "ok");
}

static ERL_NIF_TERM dht11_read(ErlNifEnv* env, int argc, const ERL_NIF_TERM argv[])
{
    ...
    if((j>=40)&&(dht11_data[4]==((dht11_data[0]+dht11_data[1]+dht11_data[2]+dht11_data[3])& 0xFF))) {
        return enif_make_tuple(env, 2,
                ...
                enif_make_tuple(env, 2,
                    enif_make_double(env, dht11_data[0]),
...
ERL_NIF_INIT(Elixir.DHT.Sensor, nif_funcs, NULL, NULL, NULL, NULL)
